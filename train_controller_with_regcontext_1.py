#!/usr/bin/env python
# scripts_nuevos/train_controller_with_regcontext_1.py
"""
Controller con contexto (AE + REG):
- Reconstruye features ricas retrospectivas (con / sin lambda_proxy).
- Normaliza con mu/sd GUARDADAS en cada ckpt (AE y REG).
- Extrae z_AE + (pooled_REG, yhat_REG) como contexto CONGELADO.
- Entrena una policy (MLP o TRANSFORMER) sobre [z, pooled, yhat] con Sharpe o μ-λ·Var.

Salida:
  prepared/<world>/trained/CTRL_CTX_AEREG_T{T}_H{H}m_Z{Z}_IN{D}_{world}.pt
"""
...
class LSTMRegressorCompat(nn.Module):
    """Igual a tu LSTMRegressor, pero devolviendo (yhat, pooled)."""
...
class TransformerRegressorCompat(nn.Module):
    """Igual a tu TransformerRegressor, pero devolviendo (yhat, pooled)."""
...
class PolicyTransformer(nn.Module):
    """Atiende sobre 3 'tokens': z_AE, pooled_REG, yhat_REG (como token)."""
    def __init__(self, z_dim:int, h_dim:int, y_dim:int=1,
        ...
        self.y_proj  = nn.Sequential(nn.Linear(y_dim,   d_model), nn.LayerNorm(d_model))
        enc_layer = nn.TransformerEncoderLayer(d_model, nhead, ff, dropout,
                                               batch_first=True, norm_first=True)
        self.enc = nn.TransformerEncoder(enc_layer, num_layers=layers)
        self.attnp = AttnPool(d_model, dropout=dropout)
    ...
    def forward(self, z, h, yval):
        t1 = self.z_proj(z).unsqueeze(1)
        t2 = self.h_proj(h).unsqueeze(1)
        t3 = self.y_proj(yval.unsqueeze(-1)).unsqueeze(1)  # [B,1,D]
...
def load_regressor_from_ckpt(ckpt_path: Path, device):
    ck = torch.load(ckpt_path, map_location="cpu")
    sd = ck.get("state_dict", ck)
    ...
    if model_name=="lstm":
        ...
        net = LSTMRegressorCompat(feat_dim=feat_dim, hidden=hidden, layers=layers, dropout=dropout).to(device)
    else:
        ...
        net = TransformerRegressorCompat(feat_dim=feat_dim, d_model=d_model, nhead=nhead, ff=ff, layers=layers, dropout=dropout).to(device)
    ...
    print(f"[load REG] Cargando {len(sd_filtered)} pesos | faltan={len(missing)} | extra={len(unexpected)}")
    if unexpected:
        print(f"[load REG] Ignorando claves extra (ej.): {', '.join(unexpected[:5])}{' ...' if len(unexpected)>5 else ''}")
    net.load_state_dict(sd_filtered, strict=False)

    T = int(ck.get("context_len", ck.get("window_len", 256)))
    H = int(ck.get("horizon_min", 60))
    feat_mode = ck.get("feature_mode", "rich")
    if feat_mode != "rich":
        raise SystemExit("Este controller requiere un REG entrenado con features 'rich'.")
    feat_norm = _get_feat_norm_dict(ck)

    # info de estandarización de y
    target = ck.get("target", "zscore")       # "pct" o "zscore"
    std_y  = bool(ck.get("std_y", False))
    y_mean = float(ck.get("y_mean", 0.0))
    y_std  = float(ck.get("y_std", 1.0))

    return net, model_name, T, H, feat_norm, target, std_y, y_mean, y_std
...
def main():
    ...
    pa.add_argument("--reg-ckpt", required=True)
    pa.add_argument("--horizon-min", type=int, default=None)
    # uso de la salida del REG
    pa.add_argument("--reg-out", choices=["raw","norm"], default="raw",
                    help="raw: desestandariza usando (y_mean,y_std) del ckpt; norm: usa salida normalizada")
    # training policy
    ...
    reg_net, reg_arch, T_reg, H_reg, reg_norm, reg_target, reg_std_y, reg_y_mean, reg_y_std = \
        load_regressor_from_ckpt(Path(args.reg_ckpt), device)

    H_target = int(args.horizon_min or H_reg)
    T_run = int(max(T_ae, T_reg))  # construcción de ventanas base

    # ¿Los ckpts esperaban lambda_proxy?
    ae_F  = int(ae_norm["mu"].shape[0])  if (ae_norm  and ae_norm.get("mu")  is not None) else 9
    reg_F = int(reg_norm["mu"].shape[0]) if (reg_norm and reg_norm.get("mu") is not None) else 9
    need_lambda = (ae_F==10) or (reg_F==10)
    ...
    print(f"\n=== Controller con contexto (AE+REG) world={world.name} ===")
    print(f"Device: {device} | step_min={step_min} | T_AE={T_ae} T_REG={T_reg} → T_run={T_run} | "
          f"H_target={H_target} (REG_H={H_reg}, AE_auxH={H_aux})")
    print(f"AE: z_dim={z_dim} | REG: arch={reg_arch.upper()} | Fbase={Fbase} | need_lambda={need_lambda} | reg_out={args.reg_out}")
    ...
    X_for_reg = norm_with(reg_norm, X_win)

    # Tensores base (normalizados)
    Xae_full = torch.from_numpy(X_for_ae.astype(DT))   # [N, T_run, F]
    Xrg_full = torch.from_numpy(X_for_reg.astype(DT))  # [N, T_run, F]
    y        = torch.from_numpy(rH.astype(DT))
    if torch.cuda.is_available() and device.type == "cuda":
        Xae_full = Xae_full.pin_memory(); Xrg_full = Xrg_full.pin_memory(); y = y.pin_memory()

    # === Slicing por arquitectura: AE usa T_AE, REG usa T_REG ===
    ...
    Xrg = slice_last_T(Xrg_full, T_reg)  # [N, T_REG, F]

    # Lengths y masks por rama
    lengths_ae = np.minimum(np.arange(1, N+1), T_ae).astype(np.int64)
    lengths_rg = np.minimum(np.arange(1, N+1), T_reg).astype(np.int64)
    pad_mask_ae = np.zeros((N, T_ae), dtype=bool)
    pad_mask_rg = np.zeros((N, T_reg), dtype=bool)
    for i in range(N):
        La = lengths_ae[i]; Lr = lengths_rg[i]
        if La < T_ae: pad_mask_ae[i, :T_ae-La] = True
        if Lr < T_reg: pad_mask_rg[i, :T_reg-Lr] = True
    lengths_ae_t = torch.from_numpy(lengths_ae)
    lengths_rg_t = torch.from_numpy(lengths_rg)
    pad_mask_ae_t = torch.from_numpy(pad_mask_ae)
    pad_mask_rg_t = torch.from_numpy(pad_mask_rg)

    ae_enc.eval(); reg_net.eval()
    for p in ae_enc.parameters(): p.requires_grad_(False)
    for p in reg_net.parameters(): p.requires_grad_(False)

    @torch.no_grad()
    def encode_batch(i, end):
        xb_ae = Xae[i:end].to(device, non_blocking=True)  # [B, T_AE, F]
        xb_rg = Xrg[i:end].to(device, non_blocking=True)  # [B, T_REG, F]
        len_ae = lengths_ae_t[i:end].to(device, non_blocking=True)
        len_rg = lengths_rg_t[i:end].to(device, non_blocking=True)
        pad_ae = pad_mask_ae_t[i:end].to(device, non_blocking=True)
        pad_rg = pad_mask_rg_t[i:end].to(device, non_blocking=True)
        z = ae_enc(xb_ae)                          # [B, z_dim]
        yhat_norm, pooled = reg_net(xb_rg, len_rg, pad_rg)   # yhat en escala de entrenamiento del REG
        # Decide salida del REG
        if args.reg_out == "raw" and reg_std_y:
            yhat = yhat_norm * reg_y_std + reg_y_mean
        else:
            yhat = yhat_norm
        # Limpieza
        ...
        return z, yhat, pooled

    # Dry run para dims reales
    z0, y0, h0 = encode_batch(0, min(64, N))
    pooled_dim = int(h0.shape[1])
    in_dim = int(z_dim + 1 + pooled_dim)  # z + yhat(1) + pooled

    # Policy
    if args.policy == "transformer":
        pol = PolicyTransformer(z_dim=z_dim, h_dim=pooled_dim, y_dim=1,
    ...
    def run_epoch(mode: str, sl: slice):
        ...
        with torch.set_grad_enabled(mode=="train"):
            for i in range(start, stop, BS):
                ...
                z, yhat, h = encode_batch(i, end)
                if args.policy == "transformer":
                    a = pol(z, h, yhat)
                else:
                    feat = torch.cat([z, h, yhat.unsqueeze(-1)], dim=-1)
    ...
    for ep in range(1, args.epochs+1):
        ...
        if ep==1 or improved:
            ...
            ckpt = outdir / f"CTRL_CTX_AEREG_T{T_run}_H{H_target}m_Z{z_dim}_IN{in_dim}_{world.name}.pt"
            torch.save({
                ...
                "reg": {"ckpt": str(args.reg_ckpt), "arch": reg_arch, "context_len": int(T_reg),
                        "target": reg_target, "std_y": bool(reg_std_y), "y_mean": float(reg_y_mean), "y_std": float(reg_y_std)},
    ...
    print("✔ Entrenamiento controller con contexto (AE+REG) finalizado.")
