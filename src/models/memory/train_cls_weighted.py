#!/usr/bin/env python
# src/models/memory/train_cls_weighted.py
"""
Clasificación 3 clases (DOWN=0, NEUTRAL=1, UP=2) enfocada a señales aprendibles.

Novedades vs. versión previa:
  - Normalización rich SIN padding (usa máscara de longitudes).
  - Embargo temporal >= H entre TRAIN/VAL/TEST.
  - Etiquetado por defecto VOL-AWARE: z_H = r_H / sigma_1h(t), con tau en |z_H|.
    (Con --labeling abs_pct se usa |r_H| como antes).
  - Attention Pooling en LSTM/Transformer en lugar de media enmascarada.
  - Label smoothing opcional para CE.
...
def build_windows_from_matrix(X_all: np.ndarray, T: int) -> Tuple[np.ndarray, np.ndarray]:
    """Devuelve (X_windows[T], lengths). Left-pad con 0 antes de normalizar; devolvemos lengths para la máscara."""
    N, F = X_all.shape
...
class AttnPool(nn.Module):
    """Attention pooling simple con máscara de padding."""
...
def focal_loss(logits, targets, weight=None, alpha=None, gamma=1.0, reduction="mean"):
    logp = F.log_softmax(logits, dim=1)
    p = torch.exp(logp)
    pt = p.gather(1, targets.view(-1,1)).squeeze(1)
    logpt = logp.gather(1, targets.view(-1,1)).squeeze(1)
    w = 1.0
    if weight is not None: w = weight.gather(0, targets)
    if alpha  is not None: w = w * alpha.gather(0, targets)
    loss = - w * ((1-pt)**gamma) * logpt
...
def main():
    ...
    pa.add_argument("--focal-alpha", type=str, default="0.3,0.5,0.3")
    ...
    # r_H (porcentaje) y componentes para etiquetado vol-aware
    ...
    if not use_rich:
        # WINDOWS (legacy): [price, sigma_rolling60m] con NaNs al principio
        ...
        # Normalización (solo TRAIN) ignorando NaNs (logp y sigma)
        Xtr = X_np[idx_tr]
        ...
        legacy_stats = (m0, s0, m1, s1)  # guardamos como floats, pasaremos a tensores en-device al vuelo

        # Guardamos para tensores
        Fdim = X_np.shape[2]
        pad_mask_full = None           # en windows se calcula por NaNs cada batch
        lengths_full = None            # idem
        rich_stats = None

    else:
        # RICH: construir matriz retrospectiva y apilar ventanas de longitud T=context_len (left-pad 0)
        lam = maybe_load_lambda_proxy(world) if args.use_lambda_proxy == "auto" else None
        X_all = build_raw_features(price, STEP_MIN, lambda_proxy=lam)   # (N, F)
        H = int(args.horizon_min)
        steps_H = max(1, H // STEP_MIN)
        ...
        X_raw, lengths = build_windows_from_matrix(X_all, T=int(args.context_len))  # (N, T, F)
        Fdim = X_raw.shape[2]
        ...
        # Normalización SIN padding (usar máscara de longitudes)
        Tctx = X_raw.shape[1]
        valid = np.zeros((X_raw.shape[0], Tctx), dtype=bool)
        for i, L in enumerate(lengths):
            if L > 0:
                valid[i, Tctx - int(L):] = True
        valid_tr = valid[idx_tr]                        # (Ntr, T)
        mask3 = np.repeat(valid_tr[:, :, None], Fdim, axis=2)  # (Ntr, T, F)
        ...
        # precompute máscaras y longitudes
        pad_mask_full = ~valid  # True = pad
    ...
    if use_rich:
        ...
        # convertir stats a tensores en-device al vuelo en cada uso
        pass
    ...
    if cw_arg == "auto":
        freqs = np.bincount(y_cls[idx_tr], minlength=3).astype(np.float64)
        inv = 1.0 / np.clip(freqs, 1.0, None)
        inv = inv / inv.mean()
        inv[1] *= float(args.neutral_weight_mult)   # ajustar neutral
        class_weights = torch.tensor(inv, dtype=torch.float32, device=device)
        cw_tag = f"autoNM{args.neutral_weight_mult:g}"
    else:
        wlist = parse_vec(args.class_weights)
        assert len(wlist) == 3, "--class-weights debe ser 'auto' o tres números"
        class_weights = torch.tensor(wlist, dtype=torch.float32, device=device)
        cw_tag = "manual"

    alpha_vec = None; alpha_tag = ""
    if args.loss == "focal" and args.focal_alpha.strip():
        al = parse_vec(args.focal_alpha); assert len(al) == 3, "--focal-alpha requiere 3 valores"
        alpha_vec = torch.tensor(al, dtype=torch.float32, device=device)
        alpha_tag = "A" + "_".join(f"{v:g}" for v in al)
    ...
    loss_tag += f"_{cw_tag}" + (f"_{alpha_tag}" if alpha_tag else "")
    ckpt = outdir / f"CLS_{model_name}_{feat_tag}_{lab_tag}_{loss_tag}_H{int(args.horizon_min)}m_{world.name}.pt"

    print(f"\n=== Train CLS-weighted ({args.model}) world={world.name} H={int(args.horizon_min)}m ===")
    print(f"Device={device} | step_min={STEP_MIN} | features={args.features} | F={Fdim}")
    ...
    print(f"Loss={args.loss} | class_weights={cw_arg} | neutral_mult={args.neutral_weight_mult} | "
          f"focal_gamma={args.gamma} | focal_alpha={args.focal_alpha or 'None'} | "
    ...
    def eval_split_fn(idx_slice):
        ...
        for i in range(start, stop, BS):
            ...
            xb = X_t[i:end]
            yb = y_t[i:end].to(device, non_blocking=True)
            ...
            if use_rich:
                ...
                # windows: máscara y normalización por NaNs (logp y sigma)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for i in range(idx_tr.start or 0, idx_tr.stop, BS):
            ...
            if args.loss == "focal":
                loss = focal_loss(logits, yb, weight=class_weights, alpha=alpha_vec, gamma=args.gamma, reduction="mean")
            else:
                # CE con (opcional) label smoothing
        ...
        if ep <= args.warmup_epochs:
            if va["bal_acc"] > best_va:
                ...
                torch.save({
                    ...
                    "horizon_min": int(args.horizon_min), "variant": f"cls3_weighted_{args.features}",
                    ...
                    "loss_cfg": {"loss":args.loss, "gamma":args.gamma,
                                 "class_weights": cw_arg, "neutral_mult": args.neutral_weight_mult,
                                 "focal_alpha": args.focal_alpha, "label_smoothing": float(args.label_smoothing)},
