...
Cambios clave para series reales:
- NADA de nan_to_num ni forward-fill plano.
- Interpolación SÓLO en huecos cortos (log-precio) y segmentación por tramos válidos.
- Ventanas completas sólo dentro de segmentos válidos (sin padding).
- Warmup (WM+CLS) -> Rampa de trading -> Entrenamiento estable.
- Guardias anti-NaN/Inf y LR warmup.
...
def build_dataset_no_impute(price_raw: np.ndarray,
    ...
    """
    Devuelve:
      X_win: [M, T, F] (z-score con stats de TRAIN), ventanas completas dentro de segmentos
      rH   : [M]
      lengths: [M] (=T)
      pad_mask: [M, T] (todo False)
      norm : estadísticas (mu/sd/features)
    """
    ...
    # 5) Recolecta índices t con ventana completa [t-T+1, t] y target finito
    idx_t: List[int] = []
    for (s,e) in segments_from_valid(valid_price):
        start_t = s + (T - 1)
        end_t   = e - Hs
    ...
    if lam_use is not None: feat_names.append("lambda_proxy")
    ...
    return Xn.astype(DT), y.astype(DT), lengths, pad_mask, norm
...
def trading_loss(positions, r_future, *,
    ...
    sd = var.clamp_min(1e-12).sqrt()  # Sharpe estable
...
def main():
    ...
    pa.add_argument("--max-gap-minutes", type=int, default=60,
                    help="Huecos <= a este valor se INTERPOLAN en log-precio; mayores dividen segmento.")
    ...
    # Clasificador opcional (anti-colapso a Flat)
    ...
    pa.add_argument("--cls-gamma", type=float, default=1.5)                # focal más “fuerte” por defecto
    pa.add_argument("--cls-focal-alpha", type=str, default="0.4,0.2,0.4")  # downweight neutral
    pa.add_argument("--q-tau", type=float, default=0.5)
    pa.add_argument("--neutral-weight-mult", type=float, default=0.7,      # <1 para NO favorecer Neutral
                    help="Multiplicador para el peso de la clase Neutral (1). <1 la penaliza.")
    pa.add_argument("--class-weights", type=str, default="auto")
    pa.add_argument("--cls-prior-mode", choices=["off","uniform","empirical"], default="empirical",
                    help="Regulariza la distribución media de predicciones.")
    pa.add_argument("--cls-prior-w", type=float, default=0.05)
    pa.add_argument("--cls-temp", type=float, default=1.0,
                    help="Temperatura para probs que se pasan a la policy (no afecta a la CE).")
    pa.add_argument("--cls-weight", type=float, default=0.2)

    # Pérdidas WM/aux
    pa.add_argument("--aux-weight", type=float, default=0.2)
    pa.add_argument("--rec-weight", type=float, default=0.2)
    ...
    pa.add_argument("--pol-use-probs", action="store_true",
                    help="Si se activa, concatena softmax(logits/cls_temp) en vez de logits crudos.")

    # Trading
    pa.add_argument("--loss-type", choices=["meanvar","sharpe"], default="sharpe")
    pa.add_argument("--lambda-var", type=float, default=3.0)
    pa.add_argument("--tx-cost", type=float, default=0.0)
    pa.add_argument("--turnover-coef", type=float, default=0.0)
    ...
    # Imagination (si WM lo soporta)
    pa.add_argument("--imagine-steps", type=int, default=0)
    pa.add_argument("--imagine-weight", type=float, default=0.15)
    ...
    pa.add_argument("--warmup-epochs", type=int, default=3,
                    help="Warmup de LR; además, peso trading se rampa en este periodo.")
    pa.add_argument("--ramp-epochs", type=int, default=8,
                    help="Épocas para ramp up del peso de trading hasta 1.0.")
    ...
    X_np, rH_np, lengths_np, pad_mask_np, norm = build_dataset_no_impute(
        price_raw=price, step_min=step_min, horizon_min=args.horizon_min,
        window_len=args.window_len, lambda_proxy=lam, max_gap_minutes=args.max_gap_minutes
    )
    N, T, Fdim = X_np.shape
    ...
    if use_cls:
        if args.cls-arch if False else False:  # placate syntax highlighters; real branch below
            pass
        ...
        # Pesos de clase (auto = inverso de frecuencia). Penaliza Neutral con neutral_weight_mult < 1.
        ...
        if cw_arg == "auto":
            ...
            inv = inv / inv.mean()
            inv[1] *= float(args.neutral_weight_mult)  # downweight Neutral si <1
        ...
        # Prior para regularizar distribución media de predicciones (anti-colapso a Flat)
        if args.cls_prior_mode == "uniform":
            prior = torch.full((3,), 1.0/3.0, dtype=torch.float32, device=device)
        elif args.cls_prior_mode == "empirical":
            emp = np.bincount(y_cls_np[idx_tr], minlength=3).astype(np.float64)
            emp = emp / emp.sum()
            prior = torch.tensor(emp, dtype=torch.float32, device=device)
        else:
            prior = None

        def cls_losses(logits, yb):
            # CE / Focal
            if args.cls_loss == "focal":
                ...
                ce = (- w * ((1-pt)**args.cls_gamma) * logpt).mean()
            else:
                ce = F.cross_entropy(logits, yb, weight=class_weights)

            # Regularizador de prior (KL entre media de probs del batch y prior)
            reg = logits.new_tensor(0.0)
            if (prior is not None) and (args.cls_prior_w > 0.0):
                probs_mean = torch.softmax(logits, dim=1).mean(dim=0).clamp_min(1e-6)
                reg = args.cls_prior_w * F.kl_div(probs_mean.log(), prior, reduction="sum")
            return ce, reg
    ...
    if args.policy == "transformer":
        ...
        pol = PolicyTransformer(
            ...
            dropout=float(args.pol_drop_out) if hasattr(args, "pol_drop_out") else float(args.pol_dropout)
        ).to(device)
    else:
    ...
    print(f"Anti-Flat: loss={args.cls_loss}, neutral_mult={args.neutral_weight_mult}, prior={args.cls_prior_mode}, prior_w={args.cls_prior_w}")
    ...
    def trading_weight(ep):
        # rampa lineal hasta 1.0 en ramp_epochs (arranca en 0)
        if ep <= 0: return 0.0
        return float(min(1.0, ep / max(1, args.ramp_epochs)))

    def run_epoch(mode: str, sl: slice, ep: int):
        train_mode = (mode == "train")
        wm.train(train_mode); pol.train(train_mode)
        if use_cls: cls.train(train_mode)

        # LR warmup por época
        if train_mode:
            if ep <= args.warmup_epochs:
                set_lr(args.lr * float(ep) / float(max(1, args.warmup_epochs)))
            else:
                set_lr(args.lr)
        ...
        cls_ce_sum = 0.0; cls_reg_sum = 0.0
        skip_batches = 0

        tr_w = trading_weight(ep)
        warm_phase = (ep <= args.warmup_epochs)
        ...
        with torch.set_grad_enabled(train_mode):
            for i in range(start, stop, BS):
                ...
                if use_cls:
                    ...
                    ce, reg = cls_losses(logits, y_c[i:end].to(device, non_blocking=True))
                    loss_cls = ce + reg
                    cls_ce_sum += float(ce.detach().cpu())
                    cls_reg_sum += float(reg.detach().cpu())

                # --- Controller input
                if use_cls:
                    if args.pol_use_probs:
                        probs = torch.softmax(logits / max(1e-6, args.cls_temp), dim=1)
                        pol_in_vec = torch.cat([z, probs], dim=-1)
                    else:
                        pol_in_vec = torch.cat([z, logits], dim=-1)
                else:
                    pol_in_vec = z

                pos = pol(pol_in_vec)

                # --- Trading loss (rampa en primeras épocas)
                ...
                tr_loss = tr_w * tr_loss  # rampa

                # --- Imagination (opcional, sólo train y si WM lo soporta)
                dream_loss = xb.new_tensor(0.0)
                if train_mode and (args.imagine_steps > 0) and state is not None and wm.supports_imagination():
                    dream_state = state
                ...
                # --- Total
                loss = args.rec_weight*(wm_rec_loss + wm_kl_loss) + args.aux_weight*wm_aux_loss + tr_loss + dream_loss + (args.cls_weight*loss_cls if use_cls else 0.0)

                # Guardias anti NaN/Inf
        ...
        if use_cls:
            out["cls_loss"] = (cls_ce_sum + cls_reg_sum) / max(1, nb)
            # Métricas CLS split
            clm = eval_cls_split(cls, X, y_c, lengths_t, pad_mask_t, device, sl, BS, N)
            out["cls_acc"] = clm["acc"]; out["cls_bal"] = clm["bal_acc"]
        else:
            out["cls_loss"] = 0.0; out["cls_acc"] = 0.0; out["cls_bal"] = 0.0
        return out

    for ep in range(1, args.epochs+1):
        tr = run_epoch("train", idx_tr, ep)
        va = run_epoch("eval",  idx_va, ep)
        te = run_epoch("eval",  idx_te, ep)

        print(f"[Ep{ep:02d}] "
              f"Tr S~={tr['sharpe_like']:.3f} | Va S~={va['sharpe_like']:.3f} | Te S~={te['sharpe_like']:.3f} | "
              f"loss={tr['loss']:.4f} | WM(rec)={tr['wm_rec']:.4f} aux={tr['wm_aux']:.4f} kl={tr['wm_kl']:.4f} | "
              f"CLS(acc/bal)={tr['cls_acc']:.3f}/{tr['cls_bal']:.3f} | skipTr={tr['skip']}/{(idx_tr.stop or 0 - (idx_tr.start or 0))//BS} "
              f"skipVa={va['skip']}/{(idx_va.stop or 0 - (idx_va.start or 0))//BS} | lr={tr['lr']:.2e} | tr_w={tr['tr_w']:.2f}")
